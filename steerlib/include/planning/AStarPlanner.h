...
namespace SteerLib
{
	...
	*/
	...
	public:
		...
		AStarPlannerNode(Util::Point _point)
		{
	...
	public:
		...
		*/
		...
		/*new functions added*/
		double Euclidean(Util::Point neighbor, Util::Point targetPoint);
		double Manhattan(Util::Point neighbor, Util::Point targetPoint);
		void RetracePath(SteerLib::AStarPlannerNode startNode, SteerLib::AStarPlannerNode endNode, std::vector<Util::Point> &agentPath, SteerLib::GridDatabase2D* grid);
		void PrintOpenSet(Util::Point start, std::vector<SteerLib::AStarPlannerNode> openSet);
		void PrintNode(SteerLib::AStarPlannerNode node);
		int GetDistance(SteerLib::AStarPlannerNode nodeA, SteerLib::AStarPlannerNode nodeB);
		std::vector<SteerLib::AStarPlannerNode> GetNeighbours(SteerLib::AStarPlannerNode currentNode, SteerLib::GridDatabase2D* grid);
		void printNeighbours(std::vector<SteerLib::AStarPlannerNode> neighbours);// used for testing
	private:
