...
Util::Vector SocialForcesAgent::calcProximityForce(float dt)
{
	...
	//cout<<"Getting to calcProximityForce"<< endl;
	...
	for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbour = _neighbors.begin(); neighbour != _neighbors.end(); neighbour++)
	{
		if ( (*neighbour)->isAgent())
		{
			...
			away = away + (away_tmp *(_SocialForcesParams.sf_agent_a * 
				   exp((((this->radius() + tmp_agent->radius()) - 
				   (this->position() - tmp_agent->position()).length())/_SocialForcesParams.sf_agent_b)))*dt) ;
		//return away;
		}
		else
		{
			...
			away_obs = away_obs + (away_obs_tmp *(_SocialForcesParams.sf_wall_a * 
				   exp((((this->radius()) - (this->position() - min_stuff.second).length())/_SocialForcesParams.sf_wall_b))) * dt);
		//return away_obs;
...
Vector SocialForcesAgent::calcGoalForce(Vector _goalDirection, float _dt)
{
	...
	//cout<<"Getting to calcGoalForce";
    Util::Vector prefForce = (((_goalDirection * PREFERED_SPEED) - velocity()) / _dt);
    //Util::Vector prefForce = (((_goalDirection * PREFERED_SPEED) - velocity()) /_SocialForcesParams.sf_acceleration / _dt);
	//Util::Vector prefForce = Util::Vector(-90,0,0);
    return prefForce;
...
Util::Vector SocialForcesAgent::calcAgentRepulsionForce(float dt)
{
  ...
  // cout<<"Getting to calcAgentRepulsionForce";
    Util::Vector agent_repulsion_force = Util::Vector(0,0,0);
	...
	for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbour = _neighbors.begin(); neighbour != _neighbors.end(); neighbour++)
	{
		...
		{
			tmp_agent = dynamic_cast<SteerLib::AgentInterface *>(*neighbour);
			if (( id() != tmp_agent->id() ) && tmp_agent->computePenetration(this->position(), 					this->radius()) > .000001 )
			{
...
Util::Vector SocialForcesAgent::calcWallRepulsionForce(float dt)
{
	...
	//cout <<"Getting to calcWallRepulsionForce";
	...
	//include if in the for?
	
return wall_repulsion_force;
    //return Util::Vector(0,0,0);  //comment out?
}
